...
###################################################################
# Learning with Piecewise Box Constraints
###################################################################
...
class PWCBoxProblem:
    ...
    def __str__(self):
        return f'PWCBoxProblem-{self.num}'
    ...
    def get_lower_bound(self, X):
        ...
        area1 = (X<=-1)*(5*torch.sin(torch.pi*(X+1)/2)**2-2)
        area2 = (X>-1)*(X<=0)*(-2)
        area3 = (X>0)*(X<=1)*((4-9*(X-2/3)**2)*X-2)
        area4 = (X>1)*(3/(X)**3-2)
        return area1 + area2 + area3 + area4
    
    def get_upper_bound(self, X):
        """upper bound for inequality constraints bl(x)<=A(x)y<=bu(x)"""
        area1 = (X<=-1)*(-3*torch.sin(torch.pi*(X+1)/2))
        area2 = (X>-1)*(X<=0)*(-2)
        area3 = (X>0)*(X<=1)*(3-4*(X-0.5)**2)
        area4 = (X>1)*(2)
        return area1 + area2 + area3 + area4
    ...
    def get_resid(self, X, Y):
        ...
        return resid_lower + resid_upper # always either lower or upper bound is 0
    ...
    def get_coefficients(self, X):
        ...
        A = torch.ones(X.shape[0], 1, 1, dtype=X.dtype, device=X.device)
